...
func longTermAuth(username, realm, password string) hash.Hash {
	key := username + ":" + realm + ":" + password // implement PRECIS defined in RFC 7613
	h := md5.New()
	h.Write([]byte(key))
	return hmac.New(sha1.New, h.Sum(nil))
}
...
var rfc5769Tests = []rfc5769Test{
	// 2.1. Sample Request
	{
		...
		msg: &stun.Control{
			...
			Attrs: []stun.Attribute{
				...
				stun.ICEControlled(0x932ff9b151263b36),
				stun.Username("evtj:h6vY"),
				stun.MessageIntegrity("\x9a\xea\xa7\x0c\xbf\xd8\xcb\x56\x78\x1e\xf2\xb5" +
	...
	//  2.2. Sample IPv4 Response
	{
		...
		msg: &stun.Control{
			...
			Attrs: []stun.Attribute{
				...
				stun.MessageIntegrity("\x2b\x91\xf5\x99\xfd\x9e\x90\xc3\x8c\x74\x89\xf9" +
	...
	//  2.3. Sample IPv6 Response
	{
		...
		msg: &stun.Control{
			...
			Attrs: []stun.Attribute{
				...
				stun.MessageIntegrity("\xa3\x82\x95\x4e\x4b\xe6\x7b\xf1\x17\x84\xc9\x7c" +
	...
	//  2.4. Sample Request with Long-Term Authentication
	{
		...
		hash: longTermAuth("\u30de\u30c8\u30ea\u30c3\u30af\u30b9", "example.org", "TheMatrIX"),
